...
public class Juc04_Thread_Cas {
    ...
    static class Worker implements Runnable{
        ...
        @Override
        public void run() {
            System.out.println("请求"+Thread.currentThread().getName()+"到达预定点,准备开始抢state:)");
            try {
                cyclicBarrier.await();
                if(cas.compareAndSwapState(0,1)){
                    System.out.println("当前请求:"+Thread.currentThread().getName()+"抢到锁!");
                }else{
                    System.out.println("当前请求:+"+Thread.currentThread().getName()+",抢锁失败!");
                }
